function [ctrs1 ctrs2 P] = smoothhist2D(X,lambda,nbins,outliercutoff,plottype, minmax)
...
[n,p] = size(X);
bin = zeros(n,2);
[dum,bin(:,1)] = histc(X(:,1),edges1);
[dum,bin(:,2)] = histc(X(:,2),edges2);
%bin = bin(all(bin,2) & (bin(:,1) <= nbins(1)) & (bin(:,2) <= nbins(2)),:);
